...
@section('title', $product['title'] . ' - Pet Paradise')
        ...
        <div class="flex flex-col lg:flex-row gap-12 bg-white p-8 rounded-3xl shadow-2xl border border-gray-100">
            ...
            <div class="lg:w-1/2 flex flex-col items-center">
                ...
                <div class="w-full aspect-square overflow-hidden rounded-2xl mb-4 shadow-xl ring-1 ring-gray-100">
                    <img id="detail-main-image" src="{{ $product['image'] }}" alt="{{ $product['title'] }}" class="w-full h-full object-cover">
                </div>
                <!-- Thumbnails (Mocked) -->
            ...
            <div class="lg:w-1/2 space-y-6">
                ...
                <h1 id="detail-title" class="text-4xl font-extrabold text-gray-900">{{ $product['title'] }}</h1>

                <!-- Rating and Reviews -->
                <div class="flex items-center space-x-4">
                    <div class="flex text-lg rating-star">
                        @for ($i = 1; $i <= 5; $i++)
                            <span class="text-lg {{ $i <= ($product['rating'] ?? 4) ? 'rating-star' : 'text-gray-300' }}">&#9733;</span>
                        @endfor
                    </div>
                    <span class="text-gray-500 text-sm">{{ number_format($product['rating'] ?? 4, 1) }} ({{ $product['reviews'] ?? 0 }} reviews)</span>
                </div>
