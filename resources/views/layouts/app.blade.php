    ...
    <div id="app-container" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header class="py-4 flex justify-between items-center sticky top-0 bg-white/80 backdrop-blur-md z-10 rounded-b-xl shadow-sm px-4">
            ...
            <nav class="hidden md:flex space-x-8 text-sm font-medium">
                ...
                <a href="{{ route('services') }}" class="hover:text-orange-600 transition duration-150 py-2 @if(Route::is('services')) font-semibold text-orange-600 border-b-2 border-orange-600 @endif">Services</a>
                <a href="{{ route('about') }}" class="hover:text-orange-600 transition duration-150 py-2 @if(Route::is('about')) font-semibold text-orange-600 border-b-2 border-orange-600 @endif">About</a>
                <a href="{{ route('contact') }}" class="hover:text-orange-600 transition duration-150 py-2 @if(Route::is('contact')) font-semibold text-orange-600 border-b-2 border-orange-600 @endif">Contact</a>
                @auth
                    @if(Auth::user()->role === 'admin')
                        <a href="{{ route('admin.users') }}" class="hover:text-orange-600 transition duration-150 py-2 @if(Route::is('admin.*')) font-semibold text-orange-600 border-b-2 border-orange-600 @endif">Admin</a>
                    @endif
                @endauth
            </nav>
            ...
            <!-- Icons -->
                ...
                @php
                    // Only fetch cart count for authenticated users to improve performance
                    $cartCount = 0;
                    if (Auth::check()) {
                        $userId = Auth::id();
                        $cart = Cache::remember("cart_{$userId}", 60, function () use ($userId) {
                            return app(\App\Http\Controllers\ProductController::class)->getCartFromFirebase($userId);
                        });
                ...
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="inline mr-4">
                        @csrf
                        <button type="submit" class="text-sm text-orange-600 hover:text-orange-700">Logout</button>
                    </form>
                @else
                    <a href="{{ route('login') }}" class="text-sm text-orange-600 hover:text-orange-700 mr-4">Login</a>
                @endauth
